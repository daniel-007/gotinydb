...
func TestDB_Replication(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute*10)
	defer cancel()

	masterPath := os.TempDir() + "/" + "replicationTestMaster"
	slave1Path := os.TempDir() + "/" + "replicationTestSlave1"
	defer os.RemoveAll(masterPath)

	master, err := Open(ctx, NewDefaultOptions(masterPath))
	if err != nil {
		t.Error(err)
		return
	}
	...
	err = master.StartNetworkService()
	if err != nil {
		t.Error(err)
		return
	}

	slaveConf := NewDefaultOptions(slave1Path)
	slaveConf.AddressBindNetworkService = ":54324"
	slave, err := Open(ctx, slaveConf)
	if err != nil {
		t.Error(err)
		return
	}
	...
	err = slave.ConnectToMaster(DefaultAddressBindNetworkService)
	if err != nil {
		t.Error(err)
		return
	}
